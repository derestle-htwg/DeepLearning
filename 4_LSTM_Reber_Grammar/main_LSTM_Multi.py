...
wordSize = 7
Words, labels = make_reber_classification(5000, invalid_size=0.5, wordSize=wordSize)
print(Words)
print(labels)
# 4000 / 500
...
if __name__ == "__main__":
    ...
    adadelta_loss = Train_model(BATCH_SIZE=32, number_of_epochs=numberOfEpochs, lr=0.28, backprobOption='adadelta')
    print("Momentum optimization")
    momentum_loss = Train_model(BATCH_SIZE=32, number_of_epochs=numberOfEpochs, lr=0.017, backprobOption='momentum')
    print("RMS Propagation optimization")
    rmsprop_loss = Train_model(BATCH_SIZE=32, number_of_epochs=numberOfEpochs, lr=0.005, backprobOption='rmsprop')
    # 0.001
    # Fake data
    ...
    plt.title('LSTM Network - 1 Layer - Sigmoid - 30 epochs')
    plt.grid(True)
    plt.xlabel('Epoch')
